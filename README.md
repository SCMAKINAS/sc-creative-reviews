...
- **Video-Storage, zwei Limits:** Das Bucket-Limit (500 MB) wird vom projektweiten *Global file size limit* gedeckelt (Supabase Dashboard → Storage → Settings) — das globale Limit gilt vorrangig. Nach Projektanlage stand es auf 50 MB, Uploads >50 MB scheiterten mit `413 EntityTooLarge`, obwohl der Bucket 500 MB erlaubte. Pro-Plan: bis 500 GB einstellbar, Free-Plan: hart 50 MB. Wer das globale Limit ändert, zieht `MAX_MB` in `video/index.html` nach — das steuert die Client-Vorprüfung und den Hinweistext.
